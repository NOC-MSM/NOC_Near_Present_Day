...
parser.add_argument('-o','--outdir', help='Directory to save climatologically corrected ERA-5 2m temperature netCDF files.', default="/dssgfs01/scratch/otooth/NOC_NPD/20250116_eORCA025/NOC_Near_Present_Day/scripts/ERA5/dev/adjusted/")

# --- Configure Logging --- #
logging.basicConfig(
    filename="apply_t2m_si_adjustmet.log",
    encoding="utf-8",
    filemode="w",
...
sic_fpath = f"/dssgfs01/scratch/otooth/NOC_NPD/20250116_eORCA025/NOC_Near_Present_Day/scripts/ERA5/dev/original/{yr}/sea_ice_cover/*.nc"
sic_files = sorted(glob.glob(sic_fpath))

# Check number of t2m and sic files match:
if (n_files != len(sic_files)-1):
