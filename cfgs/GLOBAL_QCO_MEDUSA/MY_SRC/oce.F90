MODULE oce
   ...
   REAL(dp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:)  :: ssh!: SSH [m] and barotropic velocities [m/s]
   REAL(dp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   ssh_frc                    !: Forcing term in external mode for SSH [m/s]
   ...
   REAL(dp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)  :: sshn_e, ssha_e!: external ssh
   ...
   !! ---------------------
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) :: CO2Flux_out_cpl  ! Output coupling CO2 flux  
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) :: DMS_out_cpl      ! Output coupling DMS  
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) :: chloro_out_cpl   ! Output coupling chlorophyll 
                                                                ! (expected in Kg/M3)  

   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) :: PCO2a_in_cpl     ! Input coupling CO2 partial pressure 
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) :: Dust_in_cpl      ! Input coupling dust 
...
#if defined key_oasis3 && defined key_top
         ! We only actually need these arrays to be allocated if coupling and MEDUSA 
         ! are enabled
         ! jpalm -- 03-05-2022 --
         !       -- from NEMOv4, there is no more key_medusa, and ln_medusa is not yet read.
         !          allocate if key_top and key_oasis3 (it is in MEDUSA branch so should be safe enough)
         ii = ii+1
         ALLOCATE( CO2Flux_out_cpl(jpi,jpj), DMS_out_cpl(jpi,jpj),               &
                   chloro_out_cpl(jpi,jpj),                                      &
                   PCO2a_in_cpl(jpi,jpj), Dust_in_cpl(jpi,jpj),              STAT=ierr(7) )
#endif
      !
